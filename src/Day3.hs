...
numberPositons _ _ []=[]
numberPositons line pos li
    | head li == '\n' = numberPositons (line+1) 0 (tail li)
    | isDigit (head li) = 
        ((line, pos), (length . takeWhile isDigit) li, readInt $ takeWhile isDigit li)
        : numberPositons line (pos + (length . takeWhile isDigit) li) (dropWhile isDigit li)
    | otherwise = numberPositons line (pos+1) (tail li)

mapSymbols :: Int -> Int -> String -> [(Int,Int)]
mapSymbols _ _ []=[]
mapSymbols line pos (x:xs)
    | x == '\n' = mapSymbols (line+1) 0 xs 
    | isDigit x || x == '.' 
        = mapSymbols line (pos+1) xs 
    | otherwise = add3by3Symbol (line, pos) 
        ++ mapSymbols line (pos+1) xs
